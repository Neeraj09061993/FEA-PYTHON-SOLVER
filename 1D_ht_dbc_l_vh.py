...
filename = "mesh\Line_3_elements_vary_steps.msh"
...
coeff_obj = eaproperties(nel=n_ele, e=70., a=1., b=1.)
coeffs = coeff_obj.get_coeffs()
K=(coeffs['e']) #Thermal Conductivity
A=(coeffs['a']) #Cross-sectional Area
P=0.0 # Perimeter at position x
beta=10.0 #Film coefficient
T_surr=50.0 #Surrounding Temperature
g=K*A
c=-1.*P*beta
sdbc = 200. #Start Dirchlet Boundary Conditions
edbc=0. #End Dirchlet Boundary Conditions
H= beta*T_surr
q=0. #Heat generated
F=500.
K= np.array([[7000.],[4000.],[2000.]])


#Analytical_Solution
X=sym.Symbol('X')
u_ana = np.array([[200.],[199.58],[198.67],[195.76]])
